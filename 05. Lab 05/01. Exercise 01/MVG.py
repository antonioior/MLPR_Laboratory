...
def MVG(DTR, LTR, DTE, LTE, printData=False):
    ...
    logPost_MVG_pre_computed = np.load("../Solutions/logPosterior_MVG.npy")
    ...
    maxAbsoluteError = np.abs(S_logPost - logPost_MVG_pre_computed).max()
    PVAL = S_logPost.argmax(0)

    if printData:
        print("MAIN - RESULT")
        for i in range(len(mu_covariance)):
            print(f"\tMean and covariente of property {i}:")
            print(f"\tMean:")
            printMatrix(mu_covariance[i].mu)
            print(f"\tCovarience:")
            printMatrix(mu_covariance[i].covariance)
            print(f"\tS_logPost_MVG professor:")
            printMatrix(logPost_MVG_pre_computed)
            ...
            print(f"\tMVG - Error rate: {(PVAL != LTE).sum() / float(LTE.size) * 100:.1f}")
